...
struct ProviderWidths {
    declared_in: usize, provider: usize, kind: usize,
    package: usize, source: usize, status: usize,
}
...
pub fn render_provider_table(report: &ProviderReport) -> String {
    ...
    table.set_header(vec![
        header("Line:Col"), header("Declared In"), header("Provider"),
        header("Kind"), header("Package"), header("Source"), header("Status"),
    ]);
...
fn provider_row(provider: &ProviderEntry, widths: &ProviderWidths) -> comfy_table::Row {
    comfy_table::Row::from(vec![
        ...
        wrap_cell(&provider.declared_in.display().to_string(), widths.declared_in),
...
fn package_cell(provider: &ProviderEntry, width: usize) -> Cell {
    ...
    if provider.package_name.is_some() { cell.fg(Color::Cyan) } else { cell.fg(Color::DarkGrey) }
}
...
fn source_cell(provider: &ProviderEntry, width: usize) -> Cell {
    ...
    let cell = wrap_cell(&text, width);
    if provider.source_available { cell.fg(Color::Green) } else { cell.fg(Color::Red) }
}
...
fn provider_widths() -> ProviderWidths {
    ...
    if t < 110 { ProviderWidths { declared_in: 18, provider: 20, kind: 12, package: 16, source: 18, status: 14 } }
    else if t < 150 { ProviderWidths { declared_in: 24, provider: 28, kind: 18, package: 22, source: 28, status: 14 } }
    else { ProviderWidths { declared_in: 32, provider: 38, kind: 22, package: 28, source: 40, status: 14 } }
}
