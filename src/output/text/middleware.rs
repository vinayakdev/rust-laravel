...
pub fn render_middleware_tables(report: &MiddlewareReport) -> String {
    ...
    if !report.aliases.is_empty() {
        ...
        table.set_header(vec![header("Alias"), header("Target"), header("Declared At"), header("Provider")]);
    ...
    if !report.groups.is_empty() {
        let mut table = new_table();
        table.set_header(vec![header("Group"), header("Members"), header("Declared At"), header("Provider")]);
    ...
    if !report.patterns.is_empty() {
        let mut table = new_table();
        table.set_header(vec![header("Param"), header("Pattern"), header("Declared At"), header("Provider")]);
...
fn alias_row(alias: &MiddlewareAlias, w: &MiddlewareWidths) -> comfy_table::Row {
    comfy_table::Row::from(vec![
        ...
        wrap_cell(&format!("{}:{}:{}", alias.source.declared_in.display(), alias.source.line, alias.source.column), w.declared_at),
...
fn group_row(group: &MiddlewareGroup, w: &MiddlewareWidths) -> comfy_table::Row {
    comfy_table::Row::from(vec![
        ...
        wrap_cell(&format!("{}:{}:{}", group.source.declared_in.display(), group.source.line, group.source.column), w.declared_at),
...
fn pattern_row(pattern: &RoutePattern, w: &MiddlewareWidths) -> comfy_table::Row {
    comfy_table::Row::from(vec![
        ...
        wrap_cell(&format!("{}:{}:{}", pattern.source.declared_in.display(), pattern.source.line, pattern.source.column), w.declared_at),
...
fn middleware_widths() -> MiddlewareWidths {
    ...
    if t < 110 { MiddlewareWidths { name: 14, detail: 22, declared_at: 18, provider: 18 } }
    else if t < 150 { MiddlewareWidths { name: 18, detail: 34, declared_at: 24, provider: 24 } }
    else { MiddlewareWidths { name: 22, detail: 44, declared_at: 32, provider: 30 } }
}
