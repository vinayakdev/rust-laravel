...
pub fn run(initial_project: Option<&str>) -> Result<(), String> {
    ...
    let initial_project_root = initial_project
...
fn handle_connection(mut stream: TcpStream, state: Arc<State>) -> Result<(), String> {
    ...
    match path {
        "/" => write_response(
            ...
            &index_html(state.initial_project_root.as_deref()),
        )?,
    ...
    <div id="app" class="shell">
      <aside class="sidebar">
        <div class="brand">
          ...
          <h1>rust-php</h1>
          <p class="lede">Pick a project on the left, switch analyzers on top, and inspect the existing human-readable report output in the browser.</p>
        </div>
      ...
      <main class="workspace">
        <header class="toolbar">
          <div id="command-tabs" class="command-tabs"></div>
...
const APP_CSS: &str = r#":root {
  ...
  --panel-strong: #fffdf8;
  --line: rgba(63, 48, 28, 0.16);
  --text: #2f2417;
  --muted: #6e614c;
  --accent: #0e766e;
  --accent-soft: rgba(14, 118, 110, 0.12);
  --accent-strong: #115e59;
  --shadow: 0 20px 60px rgba(50, 35, 12, 0.12);
  --radius: 24px;
  --radius-sm: 16px;
}
...
.summary-chip {
  border-radius: 999px;
  padding: 10px 14px;
  border: 1px solid rgba(14, 118, 110, 0.18);
...
.project-button.is-active {
  background: linear-gradient(135deg, var(--accent), #0f8f85);
  color: white;
...
function renderViewInventory(payload) {
  ...
  return `
    <div class="report-stack">
      ...
      ${sectionTable("Views", report.view_count, ["View", "Kind", "Declared In"], viewRows, { minWidth: 980 })}
      ${sectionTable("Blade Components", report.blade_component_count, ["Component", "Class", "View", "Declared In"], bladeRows, { minWidth: 1180 })}
      ${sectionTable("Livewire Components", report.livewire_component_count, ["Component", "Class", "View", "Declared In"], livewireRows, { minWidth: 1180 })}
    </div>
  `;
}
