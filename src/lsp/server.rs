...
fn handle_message(state: &mut ServerState, message: Value) -> Result<Option<Value>, String> {
    ...
    match method {
        ...
        Some("workspace/executeCommand") => Ok(id.map(|id| {
            match execute_command_result(state, message.get("params")) {
...
fn code_action_result(state: &ServerState, params: Option<&Value>) -> Value {
    ...
    let Some((_uri, source, line, character)) =
        source_and_position_from_range(state, Some(params))
    else {
        return Value::Array(Vec::new());
    };
    let Some(index) = &state.index else {
...
fn source_and_position_from_range<'a>(
    ...
    let line = params.pointer("/range/start/line").and_then(Value::as_u64)? as usize;
    let character = params.pointer("/range/start/character").and_then(Value::as_u64)? as usize;
...
fn open_in_zed(path: &Path) -> Result<(), String> {
    ...
    log_lsp_event(format!("openInZed trying command=zed path={}", path.display()));
