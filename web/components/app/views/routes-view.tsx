...
export function RoutesView({
  ...
  const rows = report.routes.map((route) => {
    const mw = route.resolved_middleware.length
    ...
    return [
      ...
      route.controller_target ? (
        <div className="flex min-w-[180px] flex-col gap-1">
          <Badge
            variant={
              route.controller_target.accessible_from_route
                ? "default"
                : "destructive"
            }
          ...
          <TruncCell
            ...
            maxW="max-w-[220px]"
          />
          {route.controller_target.method_declared_in &&
          route.controller_target.method_line ? (
            <LocCell
      ...
      /* Middleware */
      mw.length ? (
        <div className="flex min-w-[120px] flex-wrap gap-0.5">
          {mw.map((m, i) => (
            <Badge
              key={i}
              variant="outline"
      ...
      <div className="flex min-w-[140px] flex-col gap-0.5">
        ...
        <LocCell
          file={route.registration.declared_in}
          line={route.registration.line}
          col={route.registration.column}
          root={root}
        />
        {route.registration.provider_class && (
          <TruncCell
            value={route.registration.provider_class}
            maxW="max-w-[180px]"
            muted
          />
        )}
