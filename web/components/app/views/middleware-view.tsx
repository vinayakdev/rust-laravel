...
function SrcCell({ source, root }: { source: RegistrationSource; root?: string }) {
  return (
    <div className="flex flex-col gap-0.5">
      {source.provider_class && <TruncCell value={source.provider_class} maxW="max-w-[200px]" size="text-[0.7rem]" />}
...
export function MiddlewareView({ payload }: { payload: Payload }) {
  ...
  const groupRows = report.groups.map((g) => [
    ...
    g.members.length ? (
      <div className="flex flex-wrap gap-0.5 min-w-[160px]">
        {g.members.map((m, i) => (
  ...
  const patternRows = report.patterns.map((p) => [
    <code className="font-mono text-[0.75rem] font-semibold">{p.parameter}</code>,
  ...
  return (
    <div className="flex flex-col gap-4">
      <DataSection
        ...
        columns={[{ key: "alias", label: "Alias" }, { key: "target", label: "Target Class" }, { key: "declared", label: "Declared In" }]}
        rows={aliasRows}
      />
      <DataSection
        title="Middleware Groups"
        count={report.group_count}
        columns={[{ key: "group", label: "Group" }, { key: "members", label: "Members" }, { key: "declared", label: "Declared In" }]}
        rows={groupRows}
      />
      <DataSection
        title="Route Patterns"
        count={report.pattern_count}
        columns={[{ key: "param", label: "Parameter" }, { key: "pattern", label: "Pattern (Regex)" }, { key: "declared", label: "Declared In" }]}
        rows={patternRows}
      />
    </div>
  )
