...
function MethodPill({ method }: { method: string }) {
  ...
  return (
    <span
      className={`inline-flex items-center rounded border px-1.5 py-px font-mono text-[0.62rem] leading-none font-bold tracking-wide ${c}`}
    >
      {method}
    </span>
  )
...
export function RouteCompareView({ payload }: { payload: Payload }) {
  ...
  const stats = [
    ...
    {
      label: "Matched",
      value: c.matched_count,
      color: "text-green-600 dark:text-green-400",
    },
    {
      label: "Runtime Only",
      value: c.runtime_only_count,
      color: "text-amber-600 dark:text-amber-400",
    },
    {
      label: "Analyzer Only",
      value: c.analyzer_only_count,
      color: "text-amber-600 dark:text-amber-400",
    },
    {
      label: "Status",
      value: c.runnable ? "OK" : "No PHP",
      color: c.runnable
    ...
    <div className="flex flex-col gap-4">
      <Alert>
        ...
        <AlertDescription className="text-xs">
          {c.note}
          {c.artisan_path && (
            <code className="ml-2 font-mono text-[0.68rem]">
              {c.artisan_path}
            </code>
          )}
        </AlertDescription>
      ...
      <div className="grid grid-cols-3 gap-2 sm:grid-cols-6">
        {stats.map(({ label, value, color }) => (
          <div key={label} className="rounded-lg border bg-card px-3 py-2">
            <p className="text-[0.6rem] font-medium tracking-wider text-muted-foreground uppercase">
              {label}
            </p>
            <p
              className={`mt-0.5 font-mono text-base font-semibold tabular-nums ${color}`}
            >
              {value}
            </p>
          </div>
      ...
      {c.runtime_only.length > 0 && (
        <DataSection
          title="Runtime Only — Missing From Analyzer"
          count={c.runtime_only_count}
          columns={CMP_COLS}
          rows={cmpRows(c.runtime_only, root)}
        />
      )}
      {c.matched.length > 0 && (
        <DataSection
          title="Matched Routes"
          count={c.matched_count}
          columns={CMP_COLS}
          rows={cmpRows(c.matched, root)}
        />
      )}
      {c.analyzer_only.length > 0 && (
        <DataSection
          title="Analyzer Only"
          count={c.analyzer_only_count}
          columns={CMP_COLS}
          rows={cmpRows(c.analyzer_only, root)}
        />
      )}
    </div>
  )
