...
function SrcCell({ source, root }: { source: RegistrationSource; root?: string }) {
  return (
    <div className="flex flex-col gap-0.5 min-w-[140px]">
      ...
      {source.provider_class && <TruncCell value={source.provider_class} maxW="max-w-[200px]" muted />}
      <LocCell file={source.declared_in} line={source.line} col={source.column} root={root} />
    </div>
  )
}

export function ConfigView({ payload, sourceMode }: { payload: Payload; sourceMode: boolean }) {
  ...
  const rows = report.items.map((item) => [
    <div className="flex flex-col gap-0.5 min-w-[180px]">
      ...
      <LocCell file={item.file} line={item.line} col={item.column} root={root} />
    </div>,
    ...
    item.default_value != null
      ...
      : <span className="font-mono text-[0.7rem] text-muted-foreground">null</span>,
