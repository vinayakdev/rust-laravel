...
use crate::php::psr4::{
    Psr4Mapping, laravel_providers, package_name_for_source, read_json, resolve_class_file,
};
...
fn collect_class_const_fetches(
    ...
    for stmt in stmts {
        match stmt {
            ...
            Stmt::Expression { expr, .. } => collect_from_expr(*expr, source, imports, out),
            _ => {}
        }
    }
}
...
fn build_provider_entry(
    ...
    ProviderEntry {
        ...
        declared_in: strip_root(&project.root, declared_in_path),
        package_name,
        source_file: relative_source,
        source_available: source_file.is_some(),
        status: if source_file.is_some() {
            "ok".to_string()
        } else {
...
fn find_json_string_position(path: &Path, needle: &str) -> Result<(usize, usize), String> {
    ...
    if let Some(index) = text.find(&format!("\"{needle}\"")) {
        let prefix = &text[..index];
        let line = prefix.bytes().filter(|b| *b == b'\n').count() + 1;
        let column = prefix
            .rsplit('\n')
            .next()
            .map(|chunk| chunk.chars().count() + 1)
            .unwrap_or(1);
