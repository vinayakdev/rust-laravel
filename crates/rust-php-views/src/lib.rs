...
use rust_php_foundation::php::psr4::{Psr4Mapping, resolve_class_file, resolve_namespace_dir};
...
pub fn analyze(
    ...
    let usage_map = collect_view_usages(project, route_files)?;
    ...
    for stmt in stmts {
        ...
        for item in *uses {
            ...
            let key = if let Some(alias_token) = item.alias {
                span_text(alias_token.span, source)
            } else {
...
fn resolve_render_view_name(
    ...
    for stmt in program.statements.iter() {
        ...
        for member in members.iter().copied() {
            ...
            if span_text(name.span, &source) != "render" {
                continue;
            }
            for stmt in body {
                if let Stmt::Return {
