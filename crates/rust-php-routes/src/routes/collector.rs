...
use super::chain::{ChainOp, flatten_route_chain};
use crate::types::RouteRegistration;
use rust_php_foundation::overrides::FileOverrides;
use rust_php_foundation::php::ast::{expr_name, expr_to_path, strip_root};
...
pub(crate) fn collect_registered_route_files(
    ...
    let direct_files = collect_direct_route_files(project, providers, overrides)?;
    let discovered = discover_provider_route_files(project, providers, overrides)?;
...
fn has_explicit_route_registrations(
    project: &LaravelProject,
    providers: &[ProviderEntry],
    overrides: &FileOverrides,
) -> Result<bool, String> {
    ...
    if bootstrap_app.is_file()
        && !discover_bootstrap_route_files(project, &bootstrap_app, overrides)?.is_empty()
    {
        return Ok(true);
    }

    Ok(!discover_provider_route_files(project, providers, overrides)?.is_empty())
}

fn collect_direct_route_files(
    project: &LaravelProject,
    providers: &[ProviderEntry],
    overrides: &FileOverrides,
    ...
    let bootstrap_app = project.root.join("bootstrap/app.php");
    if bootstrap_app.is_file() {
        let bootstrap_files = discover_bootstrap_route_files(project, &bootstrap_app, overrides)?;
    ...
    if has_explicit_route_registrations(project, providers, overrides)? {
        return Ok(Vec::new());
    }
...
fn discover_bootstrap_route_files(
    ...
    let source = overrides.get_bytes(bootstrap_file).map_or_else(
        || {
...
fn discover_provider_route_files(
    ...
    for provider in providers {
        ...
        let source = overrides.get_bytes(&source_file).map_or_else(
            || {
                fs::read(&source_file)
...
fn extract_bootstrap_routing_args(
    ...
    for arg in args {
        ...
        let name = rust_php_foundation::php::ast::span_text(name.span, source);
